...
class Game():
    ...
    def Pi_to_Effect(self, pi, id):
        ...
        directness = np.sum([ (pi[n]-1/len(pi))**2 for n in range(len(pi)) ])*len(pi)/(len(pi)-1)
        boost = 1-directness

        Effect_s = self.Action_to_Effect(bestA, id)
        Effect_s['s/w'] = self.E_Strength*(1+boost*15)
    ...
    def to_reversed_Base3(self, Num, index, rvB3_Num=[], firsttime=True):
        ...
        if index != 0:
            rvB3_Num = self.to_reversed_Base3( Num - (rvB3_Num[index]*3**index), index-1, rvB3_Num, firsttime=False)
